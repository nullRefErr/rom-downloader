 ...
 */
/* CLIB (system malloc), not BUILTIN: BUILTIN caps LV_USE_STDLIB_MALLOC at
 * a fixed LV_MEM_SIZE pool (default 64KB) — confirmed on-device as the
 * reason box-art thumbnails silently failed to draw (a decoded 512x512
 * ARGB8888 PNG needs ~1MB, nowhere close to fitting; the allocation just
 * failed quietly, no crash, nothing logged, nothing drawn). System malloc
 * removes the need to guess a "big enough" fixed size up front. */
 ...
 *  @endcode
 */
#define LV_FONT_CUSTOM_DECLARE LV_FONT_DECLARE(lv_font_ui_14) LV_FONT_DECLARE(lv_font_ui_16) \
                               LV_FONT_DECLARE(lv_font_ui_22) LV_FONT_DECLARE(lv_font_ui_24)

/** Always set a default font */
#define LV_FONT_DEFAULT &lv_font_ui_22
